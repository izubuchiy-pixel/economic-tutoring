...
- 大学・科目別の入口（上智・GMARCH・関関同立・日東駒専・産近甲龍の19大学）: https://economic-tutoring.pages.dev/universities/
...
共通部品の生成対象はGit追跡済みHTMLと `tools/site-files.mjs` の明示的な入口ページのみです（現在41ページ）。未追跡の作業用HTMLや素材フォルダには触れません。追加16大学の個別原稿と出典は `tools/university-entry-data.mjs` と同ファイルが読み込む `tools/university-entry-data-batch2.mjs` で管理し、専用レンダラーはその16ページだけを生成します。上智・関西学院・立教と保護者ページの本文は直接編集します。
